/** Utility 모듈
 * Description: node.js의 보조적인 기능 중 유용한 기능만 모아놓은 모듈이다.
 * API 문서: https://nodejs.org/api/util.html
 * 
 * util.format(format, [...]): console.log() 메소드와 비슷하지만, console.log()는 화면에 출력하고 util.format은 문자열로 반환한다. printf와 같은 형식으로 첫 아규먼트를 사용해서 포맷팅된 문자열을 반환한다. 플레이스 홀더는 다음과 같은 아규먼트의 값으로 대체된다.
 *   - %s: 문자열
