...
var msg_count = 0;
  ...
  subscriber.on('subscribe', function(channel, count) {
    // 구독 시작 후 publisher 객체가 발행하도록 함(일치하는 채널만).
    publisher.publish('Goorm Channel', '발행자 첫 번째 메시지');
    publisher.publish('Goorm Channel', '발행자 두 번째 메시지');
    publisher.publish('Goorm Channel', '발행자 마지막 메시지');
  });

  // subscriber 객체가 메시지를 받으변 호출되는 함수
