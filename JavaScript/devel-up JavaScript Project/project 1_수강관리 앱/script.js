var crudApp = new function() {
    ...
    // 위 데이터를 토대로 실제로 테이블을 만들어주는 메소드
    this.createTable = () => {
    // 테이블을 만들고 데이터를 채우는 코드

        // col에 table header에 해당하는 데이터(myClass의 Key값들)을 넣어주는 코드: 비어있는 col 배열에 myClass 배열 속 객체들의 Key값들을 넣어줘야 함
        for(var i = 0; i < this.myClass.length; i++){
            // 각 객체들 속의 Key값들 순회
            for(var key in this.myClass[i]){
                // key를 col 배열에 담기(indexOf: 문자열 속의 문자열을 검색)
    ...
    for(var i = 0; i < this.myClass.length; i++){
        ...
        // table header 길이만큼 순회하며거기에 매칭되는 데이터를 가지고 오기
        ...
        btUpdate.setAttribute('onclick', 'crudApp.Update(this)'); // 이 버튼이 클릭될 때 실행할 메소드
        ...
        btSave.setAttribute('onclick', 'crudApp.Save(this)'); // 이 버튼이 클릭될 때 실행할 메소드
        ...
        btDelete.setAttribute('onclick', 'crudApp.Delete(this)'); // 이 버튼이 클릭될 때 실행할 메소드
    ...
    btCreate.setAttribute('style', 'background-color: #ED5650;');
    btCreate.setAttribute('onclick', 'crudApp.CreateNew(this)'); // 이 버튼이 클릭될 때 실행할 메소드
    ...
    this.CreateNew = (oButton) => {
        ...
        // tr 데이터에서 td 속의 key:value만 쏙쏙 뽑아서 obj 안에 저장
        for(var i = 1; i < this.col.length; i++) {
            ...
            if(td.childNodes[0].getAttribute('type') === 'text' || td.childNodes[0].tagName === 'SELECT') {
                ...
                if(txtVal != '') {
                    ...
                    obj = '';
                    alert('모든 항목을 입력하셔야 해요...');
                    break;
                }
            }
        }
        obj[this.col[0]] = this.myClass.length + 1; // 자동으로 새 ID 값이 부여되어 obj 0번째 인덱스에 담긴다.
    ...
    this.Update = (oButton) => {
        ...
        for(var i = 1; i < this.col.length; i++) {
            // 기존에 입력한 데이터들을 담은 Input/select를 띄워주기
            if(i == 2) {
                var td = trData.getElementsByTagName("td")[i];
                var select = document.createElement("select");
        ...
        btSave.setAttribute('style', 'display: block; bakcground-color: #2DBF64;');
        oButton.setAttribute('style', 'display: none;');
    }
    // Save 하기 - 변경된 값 저장하기(Create와 거의 비슷함)
    this.Save = (oButton) => {
        var writtenIdx = oButton.parentNode.parentNode.rowIndex;
        var trData = document.getElementById('classTable').rows[writtenIdx];
